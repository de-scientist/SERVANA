...
export class PayoutController {
  ...
  @Get('dashboard')
  async adminDashboard(@CurrentUser() user: { sub: string; role: string }, @Query('page') page = '1', @Query('pageSize') pageSize = '20', @Query('status') status?: string, @Query('providerId') providerId?: string) {
  ...
  @Auth('PROVIDER', 'ADMIN', 'SUPER_ADMIN', 'SUPPORT')
  @Post('methods')
  async addMethod(@CurrentUser() user: { sub: string; role: string }, @Body(new ZodValidationPipe(z.object({ type: z.enum(['MPESA', 'BANK']), detailsRef: z.string().min(1).max(200), isDefault: z.boolean().optional(), providerId: z.string().min(1).optional() }))) body: any) {
    const actor = { sub: user.sub, role: user.role as any };
